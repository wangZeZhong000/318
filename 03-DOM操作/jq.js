...
(function(window){
    ...
    jQuery.fn=jQuery.prototype={
        ...
        F:function(selector){
            ...
            //规避掉selector是null或者undefined的情况
            if(selector==null) return this;

            //selector有2种情况，分别表示一个DOM元素，或者是一个选择器，对于不同的功能需要进行分支判断
            ///     DOM元素是对象类型，选择器一定是字符串类型，所以可以根据参数类型判断功能

            if(jQuery.isString(selector)){//这里可以正常访问$.isString，因为F方法是在用户通过入口函数来进行访问，在访问入口函数的时候，框架中的代码已经加载完毕，$.isString方法已经存在
                ...
                this[0]=selector;
                this.length=1;
                //这是this { 0:selector,length:1 }就是一个JQ对象
            }
